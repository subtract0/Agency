...
class AgentRegistry:
    ...
    def create_instance(self, agent_id: str, config: ConfigData = None) -> str:
    ...
    def _agent_to_dict(self, agent: Agent) -> AgentData:
    ...
    def _dict_to_agent(self, data: AgentData) -> Agent:
        """Convert dict to agent from JSON storage."""
        data["status"] = AgentStatus(data["status"])
        data["created_at"] = datetime.fromisoformat(data["created_at"])
        return Agent(**data)

    def _instance_to_dict(self, instance: AgentInstance) -> AgentData:
        """Convert instance to dict for JSON storage."""
        data = asdict(instance)
        data["created_at"] = instance.created_at.isoformat()
        return data

    def _dict_to_instance(self, data: AgentData) -> AgentInstance:
        """Convert dict to instance from JSON storage."""
        data["created_at"] = datetime.fromisoformat(data["created_at"])
        return AgentInstance(**data)

    def _event_to_dict(self, event: AIQEvent) -> TelemetryData:
        """Convert event to dict for JSON storage."""
        data = asdict(event)
        data["timestamp"] = event.timestamp.isoformat()
        return data

    def _dict_to_event(self, data: TelemetryData) -> AIQEvent:
